...
exports.eventUpdated = functions.firestore
    ...
    .onUpdate(async (snapshot, context) => {
      ...
      if (before.attendees.length < after.attendees.length) {
        const attendeeJoined = after.attendees.filter((item1) => !before.attendees.some((item2) => item2.id === item1.id))[0];
      ...
      if (before.attendees.length > after.attendees.length) {
        const attendeeLeft = before.attendees.filter((item1) => !after.attendees.some((item2) => item2.id === item1.id))[0];
